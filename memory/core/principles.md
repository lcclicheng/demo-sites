...
- **三者分离（MDD）**：代码给编译器、文档给人、Memory 给 AI。AI 按 `memory/core/loading-priority.md`（P0–P8）加载——先读 `.ai/<role>` + `memory/core` + `memory/runtime/current-sprint`，再按需读 `decisions/` / `contracts/` / `tasks/` / `docs/`，不读全量。
