...
- **先读**: `memory/core/constraints.md`(PROJS / SSH / 静态边界) · `.github/workflows/*` · `deploy/vercel/`
- **约束**: `PROJS` 单一事实源；SSH 443 部署；本地 build 用 `mv` 移旧产物绕过 safe-delete。
- **上下文加载顺序**：遵守 `memory/core/loading-priority.md`（P0–P8；改部署必读 P4 `contracts/business-json.md` + `state/current-deploy.md`）
