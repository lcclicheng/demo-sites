...
export default function App() {
  ...
  const [f, setF] = useState({ name: '', email: '', service: '', stylist: '', date: '', time: '', note: '' })
  const submit = (e: React.FormEvent) => {
    ...
    window.location.href = `mailto:${d.email}?subject=${encodeURIComponent('New appointment from ' + who)}&body=${encodeURIComponent(lines)}`;
    setDone(true);
    setTimeout(() => setDone(false), 4000);
  }
  const allServices = (d.serviceCategories || []).flatMap(c => c.items)
    ...
    <div className="bg-ivory text-plum font-body">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-ivory/95 backdrop-blur-md border-b border-plum/5">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 flex items-center justify-between h-16 sm:h-20">
          ...
          <div className="hidden md:flex items-center gap-8 text-sm tracking-[0.1em] uppercase text-plum/40 font-medium">{['Services','Stylists','Book'].map(l=><a key={l} href={`#${l.toLowerCase()}`} className="hover:text-gold transition-colors duration-300">{l}</a>)}</div>
          <div className="flex items-center gap-3">
            <a href={`tel:${d.phone}`} className="hidden sm:flex items-center gap-1.5 text-sm text-plum/30 hover:text-plum/60"><Phone className="w-4 h-4"/>{d.phone}</a>
            <a href="#book" className="px-5 py-2.5 rounded-full bg-plum text-ivory text-sm font-medium hover:bg-plum/90 transition-colors">Book Now</a>
            <button onClick={()=>setMobile(true)} className="md:hidden"><Menu className="w-5 h-5"/></button>
          </div>
        </div>
      </nav>

      <AnimatePresence>{mobile&&<motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[60] md:hidden"><div className="absolute inset-0 bg-plum/40 backdrop-blur-sm" onClick={()=>setMobile(false)}/><motion.div initial={{x:'100%'}} animate={{x:0}} exit={{x:'100%'}} className="absolute right-0 top-0 bottom-0 w-72 bg-ivory flex flex-col p-6"><div className="flex justify-between mb-8"><span className="font-display text-xl italic text-plum">Menu</span><button onClick={()=>setMobile(false)}><X className="w-5 h-5"/></button></div>{['Services','Stylists','Book'].map(l=><a key={l} href={`#${l.toLowerCase()}`} onClick={()=>setMobile(false)} className="py-3 text-lg font-medium border-b border-plum/10">{l}</a>)}<a href="#book" onClick={()=>setMobile(false)} className="mt-auto w-full py-3 rounded-full bg-plum text-ivory text-center font-medium">Book Now</a></motion.div></motion.div>}</AnimatePresence>
      ...
      <div className="bg-plum text-ivory/70 py-4 text-sm"><div className="max-w-7xl mx-auto px-5 sm:px-8 flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-10">
      ...
      <section id="services" className="py-28 bg-white"><div className="max-w-6xl mx-auto px-5 sm:px-8">
        <motion.div {...fadeIn} className="mb-16"><div className="flex items-center gap-3 mb-4"><div className="h-px w-8 bg-gold/40"/><span className="text-xs tracking-[0.3em] uppercase text-gold font-semibold">Our Services</span></div><h2 className="font-display text-4xl sm:text-5xl italic text-plum">Tailored to you.</h2></motion.div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {allServices.map((s,i)=><motion.div key={i} {...fadeIn} className={`group bg-ivory rounded-xl p-6 border-l-4 border-transparent hover:border-gold hover:-translate-y-1 transition-all duration-500 ${(s as any).span===2?'md:col-span-2':''}`}>
            <div className="flex items-center gap-2 mb-3"><span className="text-[10px] text-gold font-semibold bg-gold/10 px-2 py-0.5 rounded">{s.time}</span></div>
            <h3 className="font-display text-xl italic font-semibold text-plum mb-2">{s.name}</h3>
            <p className="text-sm text-plum/60 leading-relaxed mb-4">{s.desc}</p>
            <div className="flex items-center justify-between pt-3 border-t border-plum/5"><span className="font-bold text-gold text-lg">{s.price}</span><a href="#book" className="text-xs font-semibold text-plum/30 hover:text-gold transition-colors">Book →</a></div>
          </motion.div>)}
        </div>
      </div></section>
      ...
      <section id="book" className="py-28 bg-white relative"><div className="absolute top-0 right-0 w-96 h-96 bg-gold/5 rounded-full blur-[120px]"/><div className="relative z-10 max-w-2xl mx-auto px-5 sm:px-8 text-center">
        <motion.div {...fadeIn} className="mb-12"><div className="flex items-center justify-center gap-3 mb-5"><div className="h-px w-8 bg-gold/30"/><span className="text-xs tracking-[0.3em] uppercase text-gold font-semibold">Bookings</span><div className="h-px w-8 bg-gold/30"/></div><h2 className="font-display text-4xl sm:text-5xl italic text-plum mb-4">Reserve your <span className="text-gold not-italic">time</span>.</h2></motion.div>
        <motion.form {...fadeIn} onSubmit={submit} className="bg-ivory rounded-2xl p-6 sm:p-8 border border-plum/5 text-left">
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div><label className="block text-xs font-semibold text-plum/40 uppercase tracking-wider mb-2">Name</label><input className="w-full h-12 rounded-xl border border-plum/10 px-4 text-sm bg-white/70 focus:outline-none focus:border-gold/50" value={f.name} onChange={e=>setF({...f,name:e.target.value})} required/></div>
            <div><label className="block text-xs font-semibold text-plum/40 uppercase tracking-wider mb-2">Email</label><input type="email" className="w-full h-12 rounded-xl border border-plum/10 px-4 text-sm bg-white/70 focus:outline-none focus:border-gold/50" value={f.email} onChange={e=>setF({...f,email:e.target.value})} required/></div>
          </div>
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div><label className="block text-xs font-semibold text-plum/40 uppercase tracking-wider mb-2">Service</label><select className="w-full h-12 rounded-xl border border-plum/10 px-4 text-sm bg-white/70 focus:outline-none focus:border-gold/50" value={f.service} onChange={e=>setF({...f,service:e.target.value})} required><option value="">Select</option>{allServices.map(s=><option key={s.name}>{s.name}</option>)}</select></div>
            <div><label className="block text-xs font-semibold text-plum/40 uppercase tracking-wider mb-2">Stylist</label><select className="w-full h-12 rounded-xl border border-plum/10 px-4 text-sm bg-white/70 focus:outline-none focus:border-gold/50" value={f.stylist} onChange={e=>setF({...f,stylist:e.target.value})}><option value="">Any available</option>{(d.stylists || []).map(s=><option key={s.name}>{s.name}</option>)}</select></div>
          </div>
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div><label className="block text-xs font-semibold text-plum/40 uppercase tracking-wider mb-2">Date</label><input type="date" className="w-full h-12 rounded-xl border border-plum/10 px-4 text-sm bg-white/70 focus:outline-none focus:border-gold/50" value={f.date} onChange={e=>setF({...f,date:e.target.value})} required/></div>
            <div><label className="block text-xs font-semibold text-plum/40 uppercase tracking-wider mb-2">Time</label><select className="w-full h-12 rounded-xl border border-plum/10 px-4 text-sm bg-white/70 focus:outline-none focus:border-gold/50" value={f.time} onChange={e=>setF({...f,time:e.target.value})} required><option value="">Select</option>{['9:00','10:00','11:00','12:00','13:00','14:00','15:00','16:00'].map(t=><option key={t}>{t}</option>)}</select></div>
          </div>
          <button type="submit" className="w-full py-4 rounded-full bg-plum text-ivory font-semibold text-sm hover:bg-plum/90 active:scale-[0.98] transition-all flex items-center justify-center gap-2">{done?<><Check className="w-4 h-4"/>Confirmed!</>:<>Confirm Booking <Send className="w-4 h-4"/></>}</button>
        </motion.form>
      </div></section>

      <footer className="bg-plum text-ivory py-16"><div className="max-w-7xl mx-auto px-5 sm:px-8"><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 pb-12 border-b border-ivory/10">
        <div><div className="flex items-center gap-3 mb-4"><div className="w-px h-6 bg-gold"/><h3 className="font-display text-xl italic">{d.name}</h3></div><p className="text-xs text-ivory/50 leading-relaxed">{d.tagline}</p></div>
        <div><h4 className="text-xs font-bold uppercase tracking-[0.2em] text-ivory/40 mb-4">Services</h4><div className="space-y-2 text-xs text-ivory/40">{allServices.slice(0,5).map(s=><p key={s.name}>{s.name}</p>)}</div></div>
        <div><h4 className="text-xs font-bold uppercase tracking-[0.2em] text-ivory/40 mb-4">Hours</h4><div className="space-y-2 text-xs text-ivory/40"><p>{d.hours}</p></div></div>
        <div><h4 className="text-xs font-bold uppercase tracking-[0.2em] text-ivory/40 mb-4">Find Us</h4><div className="space-y-2 text-xs text-ivory/40">{(d.street || '').split(',').map((s:any,i:number)=><p key={i}>{s.trim()}</p>)}<a href={`tel:${d.phone}`} className="block text-gold hover:text-gold/80 mt-3 text-xs font-semibold">{d.phone}</a></div></div>
      </div><p className="text-center text-[10px] text-ivory/20 pt-8">© {new Date().getFullYear()} {d.name}. {d.establishedText}</p></div>
