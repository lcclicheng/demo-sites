...
const fadeView=(dx:number)=>({initial:{opacity:0,y:24},whileInView:{opacity:1,y:0},viewport:{once:true,margin:'-30px'},transition:{duration:0.7,ease:[0.25,0.1,0.25,1],delay:dx}})
...
export default function App() {
  ...
  const [f,setF]=useState({name:'',email:'',cls:'',date:''})
  const submit = (e: React.FormEvent) => {
    ...
    window.location.href = `mailto:${d.email}?subject=${encodeURIComponent('New class enquiry from ' + who)}&body=${encodeURIComponent(lines)}`;
    setDone(true);
  ...
  return (
    <div className="bg-cream text-sage-900 font-body">
      <style dangerouslySetInnerHTML={{__html:`body{background:#f9faf7;color:#2d3a28;font-family:'Inter',system-ui,sans-serif}`}}/>
      <nav className="fixed top-0 left-0 right-0 z-50 bg-cream/95 backdrop-blur-md border-b border-sage-100"><div className="max-w-7xl mx-auto px-5 sm:px-8 flex items-center justify-between h-16 sm:h-20">
        <a href="#" className="font-display text-xl sm:text-2xl text-sage-800 tracking-[0.1em]">{d.name}</a>
        <div className="hidden md:flex items-center gap-8 text-sm font-medium text-sage-600">{['Classes','Teachers','Book'].map(l=><a key={l} href={`#${l.toLowerCase()}`} className="hover:text-sage-900 transition-colors">{l}</a>)}</div>
        <div className="flex items-center gap-3"><a href={`tel:${d.phone}`} className="hidden sm:flex items-center gap-1.5 text-sm text-sage-500"><Phone className="w-4 h-4"/>{d.phone}</a><a href="#book" className="px-5 py-2.5 rounded-full bg-sage-800 text-white text-sm font-medium hover:bg-sage-700">Book Now</a><button onClick={()=>setMobile(true)} className="md:hidden"><Menu className="w-5 h-5"/></button></div>
      </div></nav>
      <AnimatePresence>{mobile&&<motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[60] md:hidden"><div className="absolute inset-0 bg-sage-900/40 backdrop-blur-sm" onClick={()=>setMobile(false)}/><motion.div initial={{x:'100%'}} animate={{x:0}} exit={{x:'100%'}} className="absolute right-0 top-0 bottom-0 w-72 bg-cream flex flex-col p-6"><div className="flex justify-between mb-8"><span className="font-display text-xl text-sage-800">Menu</span><button onClick={()=>setMobile(false)}><X className="w-5 h-5"/></button></div>{['Classes','Teachers','Book'].map(l=><a key={l} href={`#${l.toLowerCase()}`} onClick={()=>setMobile(false)} className="py-3 text-lg font-medium border-b border-sage-100">{l}</a>)}<a href="#book" onClick={()=>setMobile(false)} className="mt-auto w-full py-3 rounded-full bg-sage-800 text-white text-center font-medium">Book Now</a></motion.div></motion.div>}</AnimatePresence>

      {/* HERO */}
      <section className="relative min-h-screen flex items-center overflow-hidden">
        <HeroBackdrop signature="breath" mood={(d as any).mood?.deco} />
      ...
      {/* INFO */}
      <div className="bg-sage-900 text-cream/80 py-4 text-sm"><div className="max-w-7xl mx-auto px-5 sm:px-8 flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-10">
        <span className="flex items-center gap-2"><MapPin className="w-4 h-4 text-emerald-300"/> {d.street}</span>
        <span className="flex items-center gap-2"><Clock className="w-4 h-4 text-emerald-300"/> {d.hours}</span>
        <span className="flex items-center gap-2"><Phone className="w-4 h-4 text-emerald-300"/> {d.phone}</span>
      </div></div>
      ...
      <section id="classes" className="py-24 sm:py-32 bg-white section-rule"><div className="max-w-6xl mx-auto px-5 sm:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {(d.classes || []).map((c,i)=><motion.div key={i} {...fadeView(i*0.05)} className="group bg-cream rounded-3xl p-6 border border-sage-100 hover:border-emerald-200 hover:-translate-y-1 transition-all duration-300">
            ...
            <h3 className="font-display text-xl italic font-semibold mb-2 text-sage-800">{c.name}</h3>
            <p className="text-sm text-sage-600 leading-relaxed mb-4">{c.desc}</p>
            <div className="flex items-center justify-between"><span className="font-bold text-sage-700 text-lg">{c.price}</span><a href="#book" className="text-xs font-semibold text-sage-400 hover:text-sage-700">Book →</a></div>
          </motion.div>)}
        </div>
      </div></section>
      ...
      {/* FOOTER */}
      <footer className="bg-sage-900 text-cream py-16"><div className="max-w-7xl mx-auto px-5 sm:px-8"><div className="grid grid-cols-2 sm:grid-cols-4 gap-8 pb-12 border-b border-cream/10"><div><h3 className="font-display text-xl text-cream tracking-[0.1em] mb-4">{d.name}</h3><p className="text-xs text-cream/50 mb-4">{d.tagline}</p><div className="flex gap-2"><a href="#" className="w-8 h-8 rounded-full bg-cream/10 flex items-center justify-center hover:bg-emerald-400/20"><Instagram className="w-3.5 h-3.5 text-cream/50"/></a></div></div><div><h4 className="text-xs font-bold uppercase tracking-[0.2em] text-cream/40 mb-4">Classes</h4><div className="space-y-2 text-xs text-cream/40">{(d.classes || []).map(c=><p key={c.name}>{c.name}</p>)}</div></div><div><h4 className="text-xs font-bold uppercase tracking-[0.2em] text-cream/40 mb-4">Hours</h4><div className="space-y-2 text-xs text-cream/40">{(d.hours || '').split('·').map((h,i)=><p key={i}>{h.trim()}</p>)}</div></div><div><h4 className="text-xs font-bold uppercase tracking-[0.2em] text-cream/40 mb-4">Find Us</h4><div className="space-y-2 text-xs text-cream/40">{(d.street || '').split(',').map((s,i)=><p key={i}>{s.trim()}</p>)}<a href={`tel:${d.phone}`} className="block text-cream/70 hover:text-emerald-300 mt-3">{d.phone}</a></div></div></div><p className="text-center text-[10px] text-cream/20 pt-8">© {new Date().getFullYear()} {d.name}. All rights reserved.</p></div>
