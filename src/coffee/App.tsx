...
const fadeView = (dx:number)=>({initial:{opacity:0,y:40},whileInView:{opacity:1,y:0},viewport:{once:true,margin:'-50px'},transition:{duration:0.8,ease:[0.22,1,0.36,1],delay:dx}})
...
export default function App() {
  ...
  const [order,setOrder]=useState({item:'',name:'',phone:'',note:''})
  const handleOrder = (e: React.FormEvent) => {
    e.preventDefault();
    const lines = Object.entries(order).filter(([, v]) => v != null && String(v).trim() !== '').map(([k, v]) => `${k.charAt(0).toUpperCase() + k.slice(1)}: ${v}`).join('\n');
    const who = order.name || order.phone || 'your website';
  ...
  return (
    <div className="bg-cream text-ink font-body">
      <nav className="fixed top-0 left-0 right-0 z-50 bg-cream/95 backdrop-blur-md border-b border-ink/5">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 flex items-center justify-between h-16">
          <div className="flex items-center gap-3"><div className="w-7 h-7 rounded-full bg-ink flex items-center justify-center"><CircleDot className="w-4 h-4 text-cream"/></div><span className="font-display text-xl tracking-tight">{(d.name || '').toUpperCase()}</span></div>
          <div className="hidden md:flex items-center gap-8 text-sm font-medium text-ink/60">{['Menu','Roastery','Visit'].map(l=><a key={l} href={`#${l.toLowerCase()}`} className="hover:text-ink transition-colors">{l}</a>)}</div>
          <div className="flex items-center gap-3"><a href="#order" className="hidden sm:flex items-center gap-2 px-5 py-2 rounded-full bg-vermillion text-white text-sm font-medium hover:bg-red-700 transition-colors">Order Now <Coffee className="w-4 h-4"/></a><button onClick={()=>setMobileOpen(true)} className="md:hidden"><Menu className="w-5 h-5"/></button></div>
        </div>
      </nav>
      <AnimatePresence>{mobileOpen&&(
        <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[60] md:hidden">
          ...
          <motion.div initial={{x:'100%'}} animate={{x:0}} exit={{x:'100%'}} className="absolute right-0 top-0 bottom-0 w-72 bg-cream flex flex-col p-6">
            <div className="flex justify-between mb-8"><span className="font-display text-xl">Menu</span><button onClick={()=>setMobileOpen(false)}><X className="w-5 h-5"/></button></div>
            {['Menu','Roastery','Visit'].map(l=><a key={l} href={`#${l.toLowerCase()}`} onClick={()=>setMobileOpen(false)} className="py-3 text-lg font-medium border-b border-ink/10">{l}</a>)}
            <a href="#order" onClick={()=>setMobileOpen(false)} className="mt-auto w-full py-3 rounded-full bg-vermillion text-white text-center font-medium">Order Now</a>
          </motion.div>
        </motion.div>
      )}</AnimatePresence>
      ...
      {/* INFO BAR */}
      <div className="bg-ink text-cream/80 py-4 text-sm">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-10">
          <span className="flex items-center gap-2"><MapPin className="w-4 h-4 text-vermillion"/> {d.street}</span>
          <span className="flex items-center gap-2"><Clock className="w-4 h-4 text-vermillion"/> {d.hours}</span>
          <span className="flex items-center gap-2"><Phone className="w-4 h-4 text-vermillion"/> {d.phone}</span>
        </div>
      </div>
      ...
      <section id="menu" className="py-24 sm:py-32 bg-white">
        <div className="max-w-6xl mx-auto px-5 sm:px-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {(d.menuItems || []).map((c,i)=>(
              <motion.div key={i} {...fadeView(i*0.1)} className="group relative bg-cream rounded-3xl p-6 sm:p-8 border border-ink/5 hover:border-vermillion/20 transition-all duration-500 hover:shadow-lg">
                <div className="flex items-center gap-2 mb-4"><span className="text-[10px] tracking-[0.2em] uppercase text-vermillion font-bold">{c.roast}</span><div className="flex-1 h-px bg-ink/10"/><span className="text-xs text-stone">{c.origin}</span></div>
                <div className="flex items-start justify-between mb-3"><h3 className="font-display text-2xl sm:text-3xl">{c.name}</h3><span className="font-display text-xl text-vermillion">{c.price}</span></div>
                <p className="text-xs tracking-[0.15em] uppercase text-stone mb-4">{c.notes}</p>
                <p className="text-sm text-stone/80 leading-relaxed mb-3">{c.desc}</p>
                <div className="flex gap-1">{Array(c.roastLevel||3).fill(0).map((_,j)=><div key={j} className="w-2 h-2 rounded-full bg-vermillion/40"/>)}</div>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section id="order" className="py-24 sm:py-32 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-2xl mx-auto px-5 sm:px-8 text-center">
          ...
          <motion.form {...fadeView(0.1)} onSubmit={handleOrder} className="bg-white rounded-3xl p-6 sm:p-8 border border-ink/5">
            <div className="mb-4"><label className="block text-xs font-semibold text-ink/40 uppercase tracking-wider mb-2">Your Coffee</label><select value={order.item} onChange={e=>setOrder({...order,item:e.target.value})} className="w-full h-14 rounded-xl border border-ink/10 px-4 text-sm bg-cream/50 focus:outline-none focus:border-vermillion/50" required><option value="">Select your brew</option>{(d.menuItems || []).map(c=><option key={c.name} value={c.name}>{c.name} — {c.price}</option>)}</select></div>
            <div className="grid grid-cols-2 gap-4 mb-4"><div><label className="block text-xs font-semibold text-ink/40 uppercase tracking-wider mb-2">Name</label><input className="w-full h-14 rounded-xl border border-ink/10 px-4 text-sm bg-cream/50 focus:outline-none focus:border-vermillion/50" value={order.name} onChange={e=>setOrder({...order,name:e.target.value})} required/></div><div><label className="block text-xs font-semibold text-ink/40 uppercase tracking-wider mb-2">Phone</label><input className="w-full h-14 rounded-xl border border-ink/10 px-4 text-sm bg-cream/50 focus:outline-none focus:border-vermillion/50" value={order.phone} onChange={e=>setOrder({...order,phone:e.target.value})} required/></div></div>
            <div className="mb-6"><label className="block text-xs font-semibold text-ink/40 uppercase tracking-wider mb-2">Notes</label><textarea className="w-full rounded-xl border border-ink/10 p-4 text-sm bg-cream/50 focus:outline-none focus:border-vermillion/50 resize-none h-20" value={order.note} onChange={e=>setOrder({...order,note:e.target.value})}/></div>
            <button type="submit" className="w-full py-4 rounded-full bg-vermillion text-white font-bold text-sm hover:bg-red-700 active:scale-[0.98] transition-all flex items-center justify-center gap-2">{submitted?<><Check className="w-4 h-4"/>Order Placed!</>:<>Place Order <Send className="w-4 h-4"/></>}</button>
          </motion.form>
        </div>
      </section>
      ...
      <footer className="bg-ink text-cream">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 py-16">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 pb-12 border-b border-cream/10">
            <div className="col-span-2 sm:col-span-1"><div className="flex items-center gap-2 mb-4"><div className="w-6 h-6 rounded-full bg-vermillion flex items-center justify-center"><CircleDot className="w-3 h-3 text-cream"/></div><span className="font-display text-lg text-cream">{(d.name || '').toUpperCase()}</span></div><p className="text-xs text-cream/40 leading-relaxed">{d.tagline}</p><div className="flex gap-3 mt-5"><a href="#" className="w-8 h-8 rounded-full bg-cream/5 flex items-center justify-center hover:bg-vermillion/20 transition-colors"><Instagram className="w-3.5 h-3.5 text-cream/60"/></a><a href="#" className="w-8 h-8 rounded-full bg-cream/5 flex items-center justify-center hover:bg-vermillion/20 transition-colors"><Facebook className="w-3.5 h-3.5 text-cream/60"/></a></div></div>
            <div><h4 className="text-xs font-semibold text-cream/60 uppercase tracking-[0.2em] mb-4">Menu</h4><div className="space-y-2 text-xs text-cream/40">{(d.menuItems || []).slice(0,5).map(c=><a key={c.name} href="#" className="block hover:text-cream/70 transition-colors">{c.name}</a>)}</div></div>
            <div><h4 className="text-xs font-semibold text-cream/60 uppercase tracking-[0.2em] mb-4">Info</h4><div className="space-y-2 text-xs text-cream/40"><><a key="FAQs" href="#" className="block hover:text-cream/70 transition-colors">FAQs</a><a key="Shipping" href="#" className="block hover:text-cream/70 transition-colors">Shipping</a><a key="Returns" href="#" className="block hover:text-cream/70 transition-colors">Returns</a><a key="Privacy" href="#privacy" className="block hover:text-cream/70 transition-colors">Privacy</a><a key="Terms" href="#" className="block hover:text-cream/70 transition-colors">Terms</a></></div></div>
            <div><h4 className="text-xs font-semibold text-cream/60 uppercase tracking-[0.2em] mb-4">Find Us</h4><div className="space-y-2 text-xs text-cream/40">{(d.street || '').split(',').map((s,i)=><p key={i}>{s.trim()}</p>)}<a href={`tel:${d.phone}`} className="block text-cream/70 hover:text-vermillion mt-3">{d.phone}</a><a href={`mailto:${d.email}`} className="block text-cream/70 hover:text-vermillion">{d.email}</a></div></div>
          </div>
          <div className="flex flex-col sm:flex-row justify-between gap-4 pt-8 text-[10px] text-cream/20"><p>© {new Date().getFullYear()} {d.name}. All rights reserved.</p><p>{d.location}</p></div>
        </div>
