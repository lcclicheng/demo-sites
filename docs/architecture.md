...
GitHub Pages 是静态托管，有软限制，45 个站全静态打包后若图片未优化可能接近边界：
...
| `build-clean.sh` | 全量干净重建 45 站（含时间戳校验 + 失败闸门，防 EBUSY 假成功/漏站） | ✅ 核心 |
...
### Mood 系统（v0.10，每站美感独立深化）

数据驱动的「每站个性」层：改动集中在共享系统，每站 JSON 仅加 1 个 `mood` 字段，守住七原则（复用不定制 / 单一事实源 / theme-agnostic）。

- **字段**：example JSON 顶层 `"mood": { "deco", "hero", "sig", "cta" }`；`getMood(d)`（src/components/visual.tsx）合并默认 `{deco:'balanced',hero:'center',sig:'on',cta:'fill'}`，缺字段不报错。
- **deco → 装饰密度**：`HeroBackdrop` 接受 `mood` 并在根 div 加 `deco-*` 类；`VISUAL_CSS` 据此调 glow 透明度、隐藏粒子/呼吸环（minimal）、或加强水印（rich）。legacy 8 套模板接 `mood={(d as any).mood?.deco}`（最小安全改动）。
- **hero → 构图（仅 sectioned）**：`components/sections/Hero.tsx` 读 `mood.hero` 切 `center`(默认) / `asym`(左文案+右玻璃卡，兼容旧 `designVariant:'B'`) / `split`(编辑型两栏新布局)；并给 `SignatureMark` 加 `mood-reveal` 微交互（prefers-reduced-motion 降级）。
- **sig → 第二招牌**：`mood.sig:'on'` 在 Hero 底部插 `SignatureDivider`（editorial 规则 + 行业 motif，theme-agnostic）。
- **backdrop → 行业氛围寄存器（仅 sectioned，v0.10.1）**：`components/sections/Hero.tsx` 内 `deriveBackdrop(sig)` 依 `deriveSignature(d)` 结果把 hero 背景分两档——`structural`(law/account/estate/hotel/trades → monogram/ledger/key/grid/forge) 用 `HeroBackdrop variant="grid"` 建筑网格背景、关呼吸环与漂浮粒子（干净编辑感）；`organic/neutral`(咖啡/瑜伽/甜点/沙龙/餐厅/美甲/健身等) 用默认发光+呼吸环+粒子。`d.mood.backdrop:"grid"` 可强制网格。属借 motionsites「氛围匹配行业」思路、纯用现有 `HeroBackdrop` 工具箱，不引视频/新引擎（industry 模板各自已硬编码行业签名+背景，无需此逻辑）。
- **cta → 按钮形态**：`shared.tsx` 的 `CtaButtons` 接 `cta` 切 `fill`(实心+描边) / `outline`(双描边) / `ghost`(文字链+箭头)。
- **范围**：Day1–4 外联 20 站已加 mood（同行业对刻意错开 deco/hero/cta 保证辨识度）；其余站缺省走默认，视觉不变。

### PROJS —— 站点清单单一事实源

```
PROJS=( atelier-salon breath-yoga chambers-law creme-dessert forge-trades ... )  # 共 45 站；完整数组见 build-clean.sh（单一事实源）
```

**现 45 站**（v1.1.0 时期为 20：10 个 curated 样板预设【全部 sectioned】 + 9 个真实商家 demo + 1 个 sectioned-demo 引擎展示；现扩至 45，完整列表见 `build-clean.sh` 的 `PROJS` 数组），是 `build-clean.sh` 的硬编码数组，是「要构建的站点」**单一事实源**（见 `decisions/ADR002-json-as-source-of-truth.md`）。`validate-sites.mjs` 以它为准；两处脚本开头均有醒目提醒注释。新增真实商家站须同步加进此数组，否则 CI 孤儿闸门会阻断部署（质量闸门详见 `docs/deployment.md`）。
