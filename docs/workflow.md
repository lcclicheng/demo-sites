...
| **9 个 demo 改用真实英国商家（内容真实化）** | 中/高 | ✅ 已落地（v1.0：9 个真实英国商家 demo 全上线 GitHub Pages——morris-coffee/holborn-nails/ganache/indaba-yoga/seddons-law/gower-hotel/vale-hardware/papa-bruno/chinatown-bakery；均基于 OSM 真实商家数据 + Openverse CC 授权占位图（真图后补），命名用真实商家名同 Sotto；配套 UK Biz Finder 工具（步骤1 已做）+ 资产流水线 skill（步骤3 已做）；**步骤2（联系真实商家取真图/真数据替换）🟡 筹备中（2026-07-15 启动：外联话术 + 采集清单 + 替换 SOP 见 `docs/real-merchant-outreach.md`；待实际发信、收图、核实数据后执行替换）**） | v1.0 | 你（找真实商家+照片）/ AI（生成 demo） |
