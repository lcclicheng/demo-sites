...
| 版本 | 日期 | 关键变更 |
|---|---|---|
| **v1.1.0** | 2026-07-16 | **步骤6 Section Engine 收口 + 双轨终结（curated 层）**：10 个 curated 样板预设全部迁移到 `sectioned` 统一架构；8 套行业模板保留作真实商家垂直站 + 历史样板；站点总数 19→20（新增 morris-coffee 迁移 sectioned + sectioned-demo）；monitoring 监测点扩至 22（20 站 + 门户 + admin）；双轨在 curated 层终结 |
...
| **v0.9.2** | 2026-07-14 | **定价两档落地（进攻型）**：建站含首月无忧改版 £590 一次性（2026-07-16 由「CMS 自助」改为「首月内不限次数修改」，取消给客户交付自助后台）/ 年度呵护 £390/年，作为初始报价基准（首客交付后复盘重定），见 `docs/pricing.md`；**2026-07-15 增补分行业三档微调**（law/hotel 高、trades 经济、其余标准，±£50/±£40），见 pricing.md「分行业定价调整」 |
...
| **v0.9.6** | 2026-07-14 | **共享视觉手法再延展（第二轮抽取）**：从 20 旧站再抽 SquareMonogram（方形首字母 editorial monogram，替代圆头像，注入 salon 造型师/yoga 老师；law 已原生方形暂不动）与 HeroBackdrop variant="grid"（bento-dashboard 建筑感点阵网格纹理，注入 hotel hero）；组件库现 8 个（含 Eyebrow 待用）；全 currentColor+color-mix 主题自适应，亮暗通用 |
| **v0.9.7** | 2026-07-14 | **全模板覆盖（收口）**：restaurant 补齐为最后一站 —— 注入 HeroBackdrop(particles=false，避免与已有原生粒子重复)+ GradientText(hero 主标题)，与其余 7 套一致使用从 20 旧站采集的共享视觉系统；全 8 行业模板 / 10 个真实站点（另有 3 个开发测试用 example 因缺 slug/name 字段预存在生成时报错，非实际站点）现已统一；组件库 8 个（Eyebrow 待用）｜
| **v0.9.8** | 2026-07-14 | **Eyebrow 收口（采集闭环）**：从 20 旧站采集的共享视觉系统 8 个组件中最后一个 Eyebrow（section 小标签 kicker）接入全部 8 套模板的 section kicker（约 30 处：coffee 3 / salon 4 / dessert 5 / yoga 4 / law 4 / hotel 3 / trades 4 / restaurant 3）；保留各行业 h-px 装饰线与 accent 色（currentColor 主题自适应），restaurant 金色 hero badge（L224）保留其招牌处理；组件库 8/8 全量落地，20 旧站视觉手法采集闭环完成；全 currentColor+color-mix 主题自适应，亮暗通用｜
| **v0.9.9** | 2026-07-15 | **编辑排版层（排版/文字统一）**：从 20 旧站提炼的编辑排版规律统一 8 模板观感。generate.mjs 新增 EDITORIAL_CSS 注入每站 index.css：全局 `body{line-height:1.7}` 放松正文行高；`.measure`(44rem)/`.measure-lg`(56rem) 阅读宽度限宽；`.lead` 导语段落；`.rule`/`.section-rule` 发丝分隔线（currentColor+color-mix 主题自适应）。8 模板应用：hero 副标题加 `measure-lg`(8 处)、章节导语段落加 `lead`(约 10 处)、内部 `<section>` 加 `section-rule` 顶边线(约 26 处)，共约 44 处；避开 vanguard 视频 hero / dessert 噱头动效，仅取排版规律；全 currentColor+color-mix 主题自适应，亮暗通用｜

| **v1.0.1** | 2026-07-16 | **定价模型微调 + 步骤2 外联推进**：A 档取消 Decap CMS 自助交付（省 OAuth/培训/维护），改「建站 + 首月内不限次数修改」；价格与分行业浮动不变；4 家高风险商家首批外联发送包就绪（各带独立预览链接 + Ethan Li 署名），indaba-yoga 补未核实免责横幅；`generate.mjs` 合规注入兼容 `_source` 对象（uk-biz-finder 导出直喂）；CI 孤儿闸门、监控、冒烟测试保持 |
| **v1.0** | 2026-07-15 | **9 个真实英国商家 demo 全上线 GitHub Pages**：新增 9 个基于 OSM 真实商家数据的 demo 站（morris-coffee/holborn-nails/ganache/indaba-yoga/seddons-law/gower-hotel/vale-hardware/papa-bruno/chinatown-bakery），共 19 站在线；照片用 Openverse CC 授权占位（真图后补）；配套 UK Biz Finder 商家批量搜索工具 + 资产流水线 skill；favicon emoji 改品牌首字母 monogram；ganache 玫粉配色改深酒红高端化；严格审计 19 站 0 孤儿 |
> 完整版本记录见文末「版本记录」行。
...
**形态**：
- 一套 **Section Engine 模板引擎**（`sectioned` 组合器 + theme-agnostic 通用 section 库）+ **8 套行业模板**（`restaurant` / `coffee` / `salon` / `dessert` / `yoga` / `law` / `hotel` / `trades`，真实商家垂直站 + 历史保留）
- 当前 **20 个线上站点（双轨）**：**10 个 curated 样板预设**（全部跑在 `sectioned` 统一架构）+ **9 个真实英国商家演示站**（基于 OSM 真实数据，1 个已迁移 sectioned、8 个仍用行业模板）+ **1 个 sectioned-demo 引擎展示站**，客户先看演示判断风格
- 你**替客户填内容**（客户不懂开发），一键生成站点并部署
- 部署在 **GitHub Pages**，零服务器成本

**在线门户**：`https://lcclicheng.github.io/demo-sites/`

**当前 20 个线上站点（双轨）**，站点清单以 `build-clean.sh` 的 `PROJS`（单一事实源）为准。

**A. Curated 样板预设（10 站，已全部迁移到 `sectioned` 统一架构）**

| 源文件 | 架构 / 模板 | 线上 slug | 在线地址 |
|---|---|---|---|
| examples/atelier-salon.json | sectioned | atelier | /demo-sites/atelier/ |
| examples/breath-yoga.json | sectioned | breath | /demo-sites/breath/ |
| examples/chambers-law.json | sectioned | chambers | /demo-sites/chambers/ |
| examples/creme-dessert.json | sectioned | creme | /demo-sites/creme/ |
| examples/forge-trades.json | sectioned | forge | /demo-sites/forge/ |
| examples/mario-pizza.json | sectioned | mario | /demo-sites/mario/ |
| examples/mono-coffee.json | sectioned | mono | /demo-sites/mono/ |
| examples/patisserie-v2.json | sectioned | patisserie | /demo-sites/patisserie/ |
| examples/sotto-sotto.json | sectioned | sotto-sotto | /demo-sites/sotto-sotto/ |
| examples/vault-hotel.json | sectioned | vault | /demo-sites/vault/ |

**B. 真实英国商家演示站（9 站，基于 OSM 真实数据）**

| 源文件 | 架构 / 模板 | 线上 slug | 在线地址 |
|---|---|---|---|
| examples/morris-coffee.json | sectioned（迁移 pilot） | morris-coffee | /demo-sites/morris-coffee/ |
| examples/holborn-nails.json | salon | holborn-nails | /demo-sites/holborn-nails/ |
| examples/ganache.json | dessert | ganache | /demo-sites/ganache/ |
| examples/indaba-yoga.json | yoga | indaba-yoga | /demo-sites/indaba-yoga/ |
| examples/seddons-law.json | law | seddons-law | /demo-sites/seddons-law/ |
| examples/gower-hotel.json | hotel | gower-hotel | /demo-sites/gower-hotel/ |
| examples/vale-hardware.json | trades | vale-hardware | /demo-sites/vale-hardware/ |
| examples/papa-bruno.json | restaurant | papa-bruno | /demo-sites/papa-bruno/ |
| examples/chinatown-bakery.json | dessert | chinatown-bakery | /demo-sites/chinatown-bakery/ |

**C. Section Engine 展示站（1 站）**：`examples/sectioned-demo.json` → `sectioned` → /demo-sites/sectioned-demo/

> ℹ️ **双轨说明（v1.1.0 收口）**：curated 样板预设层已 100% 迁移到 `sectioned` 统一架构（「未来迁移」对 curated 层已完成）；8 套行业模板（`restaurant/coffee/salon/dessert/yoga/law/hotel/trades`）保留作真实商家垂直展示 + 历史样板，不删除。真实商家站多数仍用各自行业模板展示该垂直行业打磨过的 UI；`morris-coffee` 作为首个迁移 `sectioned` 的真实商家 pilot。
...
> 💡 curated 样板预设（10 站）已全部跑在 `sectioned` Section Engine（见 §1 站点表），不依赖这 8 套；新增客户站**优先走 `sectioned`**（统一数据契约 + AI 组合）。
...
| `docs/workflow.md` | **本文档（v1.1.0，git 跟踪）** | — |
...
> 当前 `PROJS=( atelier-salon breath-yoga chambers-law creme-dessert forge-trades mario-pizza mono-coffee patisserie-v2 sotto-sotto vault-hotel morris-coffee holborn-nails ganache indaba-yoga seddons-law gower-hotel vale-hardware papa-bruno chinatown-bakery sectioned-demo )` —— 共 **20 站**（10 个 curated 样板预设【全部 sectioned】 + 9 个真实商家 demo【morris-coffee 已迁 sectioned、其余 8 个用行业模板】 + 1 个 sectioned-demo 引擎展示），是 `build-clean.sh` 的硬编码数组，是「要构建的站点」**单一事实源**。`validate-sites.mjs` 以它为准；两处脚本开头均有醒目提醒注释（见 §8）。新增真实商家站须同步加进此数组，否则 CI 孤儿闸门会阻断部署（见 §8）。
...
- **场景**：客户只改一张图、一个电话或一段文案，且你只想最快验证 / 预览，不想等全量 20 站重建（约数分钟）。
...
- 详见 `docs/cms.md`（§5 全 20 站自动映射、§6 生产模型、§11 一页纸上手卡）。
...
| 定价两档（建站含首月无忧改版 £590 一次性 / 年度呵护 £390 年，含**分行业三档微调**：law/hotel 上浮、trades 下浮，幅度 ±£50/±£40；**2026-07-16 A 档取消 CMS 自助交付、改为首月内不限次数修改**） | 低/中 | ✅ 已落地（v0.9.2 两档基准；2026-07-15 增补分行业三档微调，见 `docs/pricing.md`「分行业定价调整」；2026-07-16 A 档改为首月无忧改版、取消 CMS 自助，见 pricing.md v0.9.2.2；首客交付后复盘重定） | v0.9.2 | 你（定价）/ AI（文档） |
...
| **9 个 demo 改用真实英国商家（内容真实化）** | 中/高 | ✅ 已落地（v1.0：9 个真实英国商家 demo 全上线 GitHub Pages——morris-coffee/holborn-nails/ganache/indaba-yoga/seddons-law/gower-hotel/vale-hardware/papa-bruno/chinatown-bakery；均基于 OSM 真实商家数据 + Openverse CC 授权占位图（真图后补），命名用真实商家名同 Sotto；配套 UK Biz Finder 工具（步骤1 已做）+ 资产流水线 skill（步骤3 已做）；**步骤2（联系真实商家取真图/真数据替换）🟡 推进中（2026-07-15 启动：外联话术 + 采集清单 + 替换 SOP 见 `docs/real-merchant-outreach.md`；2026-07-16 首批 4 家高风险商家发送包就绪 `docs/outreach-batch-1.md`——各带独立预览链接 + Ethan Li 署名，indaba-yoga 已补免责横幅；待实际发信、收图、核实数据后执行替换）**） | v1.0.1 | 你（找真实商家+照片）/ AI（生成 demo） |
> **配套工具链（v1.0 新增）**：① **UK Biz Finder**（`../uk-biz-finder/`，独立单文件 SPA，零构建）：按行业+英国地区批量拉取真实商家（Overpass+Photon+Openverse，免费无 key），导出 JSON 直接对齐 `examples/<slug>.json` intake 包；② **资产流水线 skill**（`~/.workbuddy/skills/uk-biz-site-asset-pipeline/`）：新增真实站时的端到端处理（Openverse 取 CC 图 + emoji→品牌首字母 monogram/lucide 图标 + THEMES/PROJS 注册 + 绕过 safe-delete + CI 孤儿闸门 + ssh 443 推送）。三步计划：步骤1（工具）✅ / 步骤3（skill）✅ / 步骤2（联系商家取真图/真数据替换）🟡 筹备中（2026-07-15 启动，外联话术 + 采集清单见 `docs/real-merchant-outreach.md`）。
