...
- **建议**：
  ...
  - **监控建议**（站点增多时务必加，避免悄悄触限）：
    - **仓库大小**：在 `deploy.yml` 加一步 `du -sh .` 与 `git count-objects -vH`，超过阈值（如 800MB）时打印告警（Actions 日志可见）；也可接第三方仓库大小监控。
...
| `docs/workflow.md` | **本文档（v0.5，git 跟踪）** | — |
...
| 项 | 难度/收益 | 状态 | 预计完成版本 |
|---|---|---|---|
| 自定义域名 / DNS / SSL / 客户交接 SOP | 低/高 | ✅ 已完成（v0.4，SOP 就绪；代码执行待真实英国客户签约） | —（SOP 已交付） |
| onboarding 工具增强（图片上传/base64、单站构建、/preview、缺图提醒、自动写 PROJS） | 低/高 | ✅ 已完成（v0.3；v0.4 自动写 PROJS；v0.5 加固） | — |
| 文档维护规则（README 回链单一事实源） | 低/中 | ✅ 已完成（v0.4） | — |
| 合规交付清单 | 低/中 | ✅ 已完成（v0.4） | — |
| SEO 基础（og:image / sitemap / robots / SITE_BASE_URL） | 中/高 | ✅ 已完成（v0.4） | — |
| 部署后通知（Slack / Telegram） | 低/中 | ✅ 已完成（v0.4，需配 secrets） | — |
| 孤儿站点软阻断（validate 默认 exit 1，可 override） | 低/中 | ✅ 已完成（v0.5） | — |
| onboard.mjs 自动写 PROJS 加固（备份 + bash -n + 还原） | 低/中 | ✅ 已完成（v0.5） | — |
