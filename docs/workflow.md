...
| **9 个 demo 改用真实英国商家（内容真实化）** | 中/高 | ✅ 已落地（v1.0：9 个真实英国商家 demo 全上线 GitHub Pages——morris-coffee/holborn-nails/ganache/indaba-yoga/seddons-law/gower-hotel/vale-hardware/papa-bruno/chinatown-bakery；均基于 OSM 真实商家数据 + Openverse CC 授权占位图（真图后补），命名用真实商家名同 Sotto；配套 UK Biz Finder 工具（步骤1 已做）+ 资产流水线 skill（步骤3 已做）；**步骤2（联系真实商家取真图/真数据替换）待用户推进**，当前搁置） | v1.0 | 你（找真实商家+照片）/ AI（生成 demo） |
> **配套工具链（v1.0 新增）**：① **UK Biz Finder**（`../uk-biz-finder/`，独立单文件 SPA，零构建）：按行业+英国地区批量拉取真实商家（Overpass+Photon+Openverse，免费无 key），导出 JSON 直接对齐 `examples/<slug>.json` intake 包；② **资产流水线 skill**（`~/.workbuddy/skills/uk-biz-site-asset-pipeline/`）：新增真实站时的端到端处理（Openverse 取 CC 图 + emoji→品牌首字母 monogram/lucide 图标 + THEMES/PROJS 注册 + 绕过 safe-delete + CI 孤儿闸门 + ssh 443 推送）。三步计划：步骤1（工具）✅ / 步骤3（skill）✅ / 步骤2（联系商家取真图/真数据替换）⏸ 搁置待用户推进。
