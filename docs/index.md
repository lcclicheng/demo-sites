...
│   └── runtime/   【易变层 P2】goals · progress · current-sprint
├── contracts/     【P4】section-data · business-json · theme（Schema 单一事实源，多 AI 统一字段）
├── state/         【运行时状态直查】current-version · release · template · build · deploy
...
- `goals.md` · `progress.md` · `current-sprint.md`（每次会话先读）

**契约 / 状态 / 决策 / 任务 / 角色（P3–P8 按需）**
- `contracts/`（Schema 单一事实源：section-data / business-json / theme）
- `state/`（运行时状态直查：version / release / template / build / deploy）
