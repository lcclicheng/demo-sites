...
> **2026-07-16 状态更新（已闭环）**：定价模型调整——A 档取消「Decap CMS 自助后台」交付，改为「首月内不限次数修改（由你代改）」。因此本 H2 原风险（9 真实商家站客户无法自助改内容、与交付承诺矛盾）**已按设计消除**：标准交付不再承诺客户自助后台，`/admin` 仅保留作内部演示能力展示，不再作为客户交付项。
