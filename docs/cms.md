...
> ⚠️ **2026-07-16 交付模型变更**：标准交付包**不再含**「客户 CMS 自助后台」——A 档改为「首月内不限次数代改」，B 档接管首月后改动（见 `docs/pricing.md` v0.9.2.2）。因此本文档**降级为内部演示能力参考 / 可选增项**，不再是客户标准交付流程；下方「生产模型」仅在客户单独付费要求自助后台时才启用。
