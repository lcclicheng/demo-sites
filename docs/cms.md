...
> 关联：`workflow.md` §5.9（交付后维护流程）、§11（客户自助 CMS 已完成 v0.8，全 20 站自动映射）。
...
> ⚠️ **交付模型说明**：A 档（首月代改）/ B 档（年费接管）标准交付包**不维护「我们运营的后台」**——v0.9.2.2 取消的是「为每个客户配 OAuth、培训、长期维护」的隐性成本（见 `docs/pricing.md`）。但 **C 档「完整买断交付」（v0.9.2.3）以 Decap 为交付机制**：把 Decap 配置指向**客户自己的仓库 + 客户自己注册的 OAuth App**，一次性 setup 后**运维全归客户**，你不再承担任何后台维护——这与 v0.9.2.2 的初衷不矛盾，只是把后台「所有权与运维」整体移交。因此本文档既是内部演示能力参考，也是 C 档买断交付的**标准执行 SOP**（§6 生产模型即 C 档交接流程）。
...
## 6. 生产模型（真实付费客户 / C 档买断交付执行流程）

> 💡 本节即 **C 档「完整买断交付」**（见 `docs/pricing.md` v0.9.2.3）的执行流程：把工程转移到客户自己的 GitHub 仓库、配置 Decap 指向该仓库，客户从此自助改内容、自行运维，你零后续维护。
...
- ✅ **全量映射**：每个站点的 `config.yml` 字段必须覆盖其 JSON 全部 key（含 `template`/`slug`），否则保存丢字段 → 构建崩。当前 `admin/config.yml` 已由 `gen-decap-config.mjs` 自动生成**全部 20 站**的完整映射（100% 覆盖）。
...
*本文档随 `workflow.md` 维护，当前状态（v0.8，站点映射已同步至 20 站）：Decap CMS 全 20 站自动映射（`gen-decap-config.mjs` 生成，100% 覆盖防丢字段）+ `template`/`slug` 已设 hidden+required + 图片 `?v=` 防缓存 + 演示站 OAuth 已激活；真实客户按 §6 生产模型各自注册 OAuth App 启用即生产可用。*
