...
/**
 * inject-privacy.mjs
 * Embeds self-contained components + hash-routes into every template App.tsx:
 *   - <PrivacyPolicy>      → #privacy
 *   - <RegisteredAddressInfo> → #address-info   (explains the UK registered office)
 * No generate.mjs changes required.
 *
 * Safe to re-run (idempotent guards on every injection).
 */
...
// ── The PrivacyPolicy component (palette-independent, inline-styled) ──
// NOTE: no backticks or ${} inside this string on purpose.
const PRIVACY_COMP = `
...
// ── The RegisteredAddressInfo component (palette-independent, inline-styled) ──
// Explains what a UK registered office address is, why it is public, and the
// options a business has when choosing one. Mirrors the Privacy component style.
const REGISTERED_INFO_COMP = `
...
// ── Regexes ──
const reactRe = /import\s*\{([^}]*)\}\s*from\s*['"]react['"]/
const dRe = /(const d = \w+)/
const mainReturnRe = /(\n)(\s*)return \((\s*\n\s*)<div/
...
function injectFile(filePath) {
  ...
  content = content.replace(reactRe, (m, p1) => {
    const names = p1.split(',').map((x) => x.trim()).filter(Boolean)
    if (!names.includes('useEffect')) { names.push('useEffect'); changed = true }
    return 'import { ' + names.join(', ') + " } from 'react'"
  })

  // 2) route state after `const d = ...`
  if (!content.includes('const [route, setRoute]')) {
    content = content.replace(dRe, (m) => m +
  ...
  // 3) early returns before main `return ( <div`
  if (!content.includes("route === '#privacy'") || !content.includes("route === '#address-info'") || !content.includes("route === '#invoice'") || !content.includes("route === '#contract'")) {
    content = content.replace(mainReturnRe, (m, nl, indent, between) => {
      let block = m
      if (!content.includes("route === '#address-info'")) {
        block = nl + indent + "if (route === '#address-info') {" + nl +
          indent + "  return (" + nl +
          indent + "    <RegisteredAddressInfo businessName={d.name} email={d.email} phone={d.phone} address={privacyAddress} />" + nl +
          indent + "  )" + nl +
          indent + "}" + block
      }
      if (!content.includes("route === '#privacy'")) {
        block = nl + indent + "if (route === '#privacy') {" + nl +
          indent + "  return (" + nl +
          indent + "    <PrivacyPolicy businessName={d.name} email={d.email} phone={d.phone} address={privacyAddress} />" + nl +
          indent + "  )" + nl +
          indent + "}" + block
      }
      if (!content.includes("route === '#invoice'")) {
        block = nl + indent + "if (route === '#invoice') {" + nl +
          indent + "  return (" + nl +
          indent + "    <Invoice businessName={d.name} email={d.email} phone={d.phone} address={privacyAddress} />" + nl +
          indent + "  )" + nl +
          indent + "}" + block
      }
      if (!content.includes("route === '#contract'")) {
        block = nl + indent + "if (route === '#contract') {" + nl +
          indent + "  return (" + nl +
          indent + "    <Contract businessName={d.name} email={d.email} phone={d.phone} address={privacyAddress} />" + nl +
          indent + "  )" + nl +
          indent + "}" + block
      }
      if (!content.includes('const privacyAddress')) {
        block = nl + indent + "const privacyAddress = d.registeredAddress || [d.street, d.location].filter(Boolean).join(', ') || 'United Kingdom'" + nl + block
      }
      return block
    })
    changed = true
  ...
  // 4) footer link (privacy)
  let footerChanged = false
  if (rel.replace(/\\/g, '/').includes('restaurant/')) {
    if (content.includes("['Privacy Policy', 'Terms', 'Accessibility']")) {
      content = content.replace(/\{\['Privacy Policy', 'Terms', 'Accessibility'\][\s\S]*?\)\)\}/,
    ...
    if (content.includes("['Privacy Policy', 'Terms of Service', 'Cookie Policy']")) {
      content = content.replace(/\{\['Privacy Policy', 'Terms of Service', 'Cookie Policy'\][\s\S]*?\)\}/,
        "<a href=\"#privacy\" className=\"hover:text-cream/60\">Privacy Policy</a>\n              <a href=\"#\" className=\"hover:text-cream/60\">Terms of Service</a>\n              <a href=\"#\" className=\"hover:text-cream/60\">Cookie Policy</a>")
      footerChanged = true
    }
  } else if (rel.replace(/\\/g, '/').includes('coffee/')) {
    if (content.includes("['FAQs','Shipping','Returns','Privacy','Terms']")) {
      content = content.replace(/\{\['FAQs','Shipping','Returns','Privacy','Terms'\][\s\S]*?\)\}/,
        "<><a key=\"FAQs\" href=\"#\" className=\"block hover:text-cream/70 transition-colors\">FAQs</a><a key=\"Shipping\" href=\"#\" className=\"block hover:text-cream/70 transition-colors\">Shipping</a><a key=\"Returns\" href=\"#\" className=\"block hover:text-cream/70 transition-colors\">Returns</a><a key=\"Privacy\" href=\"#privacy\" className=\"block hover:text-cream/70 transition-colors\">Privacy</a><a key=\"Terms\" href=\"#\" className=\"block hover:text-cream/70 transition-colors\">Terms</a></>")
      footerChanged = true
    }
  } else {
    // universal bottom link for the other templates
    if (!content.includes('href="#privacy"')) {
      const lastFooter = content.lastIndexOf('</footer>')
      if (lastFooter !== -1) {
        const linkHtml = "\n\n    <div style={{ textAlign: 'center', paddingTop: 22, borderTop: '1px solid rgba(255,255,255,0.08)', marginTop: 10 }}><a href=\"#privacy\" style={{ color: '#cbd5e1', textDecoration: 'underline', fontSize: 12, letterSpacing: '0.04em' }}>Privacy Policy</a></div>\n  "
        content = content.slice(0, lastFooter) + linkHtml + content.slice(lastFooter)
        footerChanged = true
      }
    }
  }

  // 4b) Registered Office info link (universal, independent guard)
  if (!content.includes('href="#address-info"') && content.includes('</footer>')) {
    const lastFooter = content.lastIndexOf('</footer>')
    const linkHtml = "\n\n    <div style={{ textAlign: 'center', paddingTop: 14, marginTop: 6 }}><a href=\"#address-info\" style={{ color: '#cbd5e1', textDecoration: 'underline', fontSize: 12, letterSpacing: '0.04em' }}>Registered Office Address</a></div>\n  "
    content = content.slice(0, lastFooter) + linkHtml + content.slice(lastFooter)
    footerChanged = true
  }

  // 4c) Invoice + Contract template links (universal, independent guard)
  if ((!content.includes('href="#invoice"') || !content.includes('href="#contract"')) && content.includes('</footer>')) {
    const lastFooter = content.lastIndexOf('</footer>')
  ...
  if (footerChanged) changed = true

  // 5) append components
  if (!content.includes('function PrivacyPolicy')) {
    content = content + '\n\n' + PRIVACY_COMP + '\n'
    changed = true
  }
  if (!content.includes('function RegisteredAddressInfo')) {
    content = content + '\n\n' + REGISTERED_INFO_COMP + '\n'
    changed = true
  }
  if (!content.includes('function Invoice')) {
    content = content + '\n\n' + INVOICE_COMP + '\n'
    changed = true
  }
  if (!content.includes('function Contract')) {
    content = content + '\n\n' + CONTRACT_COMP + '\n'
    changed = true
  }

  if (changed) fs.writeFileSync(filePath, content, 'utf-8')
  console.log((changed ? '✅ ' : '⏭️  ') + rel)
}

// ── Targets: root src/App.tsx + each template dir ──
const targets = [path.join(SRC, 'App.tsx')]
for (const entry of fs.readdirSync(SRC, { withFileTypes: true })) {
  if (entry.isDirectory()) {
    const f = path.join(SRC, entry.name, 'App.tsx')
    if (fs.existsSync(f)) targets.push(f)
  }
}

console.log('Injecting Privacy Policy + Registered Office info into ' + targets.length + ' files...\n')
for (const t of targets) injectFile(t)
console.log('\nDone.')
